...
@interface AudioDeviceCenter (Private)
...
@implementation AudioDeviceCenter
...
- (NSArray*)_allDevicesWithDeviceID: (AudioDeviceID)deviceID isInput: (BOOL)isInput
{
	...
	if( !AudioDeviceGetPropertyInfo(deviceID, 0, isInput, kAudioDevicePropertyDataSources, &size, NULL) )
	{
		...
		if( count )
		{
			...
			if( !AudioDeviceGetProperty(deviceID, 0, isInput, kAudioDevicePropertyDataSources, &size, list))
			{
				for (i = 0; i < count; i++)
				{
					device = [[AudioDevice alloc] initWithAudioDeviceID: deviceID source: list[i] isInput: isInput];
	...
	if( ![objList count] )
	{
		device = [[AudioDevice alloc] initWithAudioDeviceID: deviceID source: 0 isInput: isInput];
		[objList addObject: device];
	}
...
- (OSStatus)_setDefaultDeviceOfClass: (OSType)type to: (AudioDevice*)device
{
...
- (AudioDevice*)_defaultDeviceOfClass: (OSType)type
{
	AudioDevice*	device;
...
- (void)_devicePropertyChanged: (NSDictionary*)args
{
	...
	if( newDefaultDevice )
	{
		AudioDevice* newDefaultDeviceObj = [self deviceWithID: newDefaultDevice isInput: isInput];
...
static OSStatus devicePropertyChanged( AudioDeviceID deviceID, UInt32 inChannel, Boolean isInput,
                                      AudioDevicePropertyID inPropertyID, void *inClientData )
{
	AudioDeviceCenter* deviceCenter = (__bridge AudioDeviceCenter *)inClientData;
	NSDictionary* args;
		...
		if( [device coreAudioTransportType] == kIOAudioDeviceTransportTypeBuiltIn )
		{
			if( flag )
			{
				status = AudioDeviceAddPropertyListener( [device coreAudioDeviceID], 0, [device coreAudioIsInput],
                                                        kAudioDevicePropertyDataSource, devicePropertyChanged, (__bridge void *)(self));
